...
<?php 
    if(!isset($_GET['dia']) && !isset($_GET['id'])){
?>
  ...
  <?php 
    $dia = date('Y-m-d');
    $sql = "SELECT RESERVA.CODIGO AS CODIGO,RESERVA.DATA_ULTILIZAR AS DATA_USAR,DATE_FORMAT(RESERVA.DATA_ULTILIZAR,'%d/%m/%Y') AS DATA, RESERVA.DATA AS EFETUOU, USUARIO.NOME AS NOME,USUARIO.SOBRENOME AS SOBRENOME,EQUIPAMENTO_CODIGO AS EQUI_CODIGO, EQUIPAMENTO.DESCRICAO AS EQUIPAMENTO,RESERVA.AULA_CODIGO AS AULA_CODIGO, AULA.DESCRICAO AS AULA,CONCAT(AULA.DESCRICAO,'-', RESERVA.DATA) AS ORDEM, TURMA.DESCRICAO AS TURMA FROM RESERVA
...
while($rowList = mysqli_fetch_array($queryAgendamento)){
    $codigo = $rowList['EQUI_CODIGO'];
    $dia = $rowList['EFETUOU'];
    $usar = $rowList['DATA_USAR'];
    $aula = $rowList['AULA_CODIGO'];
    $sql = "SELECT COUNT(CODIGO) AS QTD FROM RESERVA WHERE EQUIPAMENTO_CODIGO = $codigo AND DATA < '$dia' AND DATA_ULTILIZAR = '$usar' AND AULA_CODIGO = $aula";
    $query = mysqli_query($con, $sql);
    echo mysqli_error($con);
    $li = mysqli_fetch_array($query);
    $qtd = $li['QTD'] + 1;
    ?>
    <tr>
      ...
      <td id="desc"><?php echo $rowList["EQUIPAMENTO"]." ".$qtd?></td>
...
<?php
  ...
  <?php 
    ...
    $_SESSION['dia'] = $dia;
    $aula = $_GET['aula'];
    $sql = "SELECT RESERVA.CODIGO AS CODIGO,RESERVA.DATA_ULTILIZAR AS DATA_USAR,DATE_FORMAT(RESERVA.DATA_ULTILIZAR,'%d/%m/%Y') AS DATA,EQUIPAMENTO_CODIGO AS EQUI_CODIGO,  RESERVA.DATA AS EFETUOU, RESERVA.AULA_CODIGO AS AULA_CODIGO, USUARIO.NOME AS NOME,USUARIO.SOBRENOME AS SOBRENOME, EQUIPAMENTO.DESCRICAO AS EQUIPAMENTO, AULA.DESCRICAO AS AULA, CONCAT(AULA.DESCRICAO,'-', RESERVA.DATA) AS ORDEM, TURMA.DESCRICAO AS TURMA FROM RESERVA
INNER JOIN EQUIPAMENTO ON EQUIPAMENTO.CODIGO = RESERVA.EQUIPAMENTO_CODIGO 
INNER JOIN USUARIO ON RESERVA.USUARIO_CPF = USUARIO.CPF
INNER JOIN AULA ON RESERVA.AULA_CODIGO = AULA.CODIGO
