...
<div id="table">
  ...
  <tbody>
      <?php 
        $sql = "SELECT * FROM OBSERVACAO INNER JOIN USUARIO ON USUARIO.CPF = OBSERVACAO.USUARIO_CPF  WHERE OBSERVACAO.USUARIO_CPF = '$cpf' ORDER BY MOMENTO DESC";
    ...
    <tr>
      ...
      <?php 
        if ($row['VISUALIZADO'] == 0) {
          
      ?>
      <td><button class="btn btn-danger">Aguardando visualização</button></td>
      <?php 
        }else if ($row['VISUALIZADO'] == 1) {
           ?>
      <td><button class="btn btn-success">Visualizado</button></td>
      <?php 
        }
      ?>
    </tr>
