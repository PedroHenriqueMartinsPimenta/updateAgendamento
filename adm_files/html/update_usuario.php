<?php 
    ...
    $cpf = $_SESSION['CPF'];
    $cpfUserSelected = $_GET['cpf'];
    $permissao = $_SESSION['PERMISSAO'];
    $sql = "SELECT * FROM USUARIO WHERE CPF = '$cpfUserSelected'";
...
<div id="page" class="site">
    ...
    <div class="col-xs col-xs-12">
        <h1 class="hero-title">
        <?php echo $row['NOME'] . " " . $row['SOBRENOME'] ?>        </h1>
    ...
    <div class="form">
        <form action="../../php/updateProfessorScript.php" method="post">
            <p>Atualizar dados do usuarios</p>
            <input type="text" required readonly value="<?php echo $row['CPF']?>" id="cpf" name="cpf" maxlength="14" onkeydown="javascript: fMasc( this, mCPF );" placeholder="CPF" class="form-control">
            <input type="text" required value="<?php echo $row['NOME']?>" id="nome" name="nome" placeholder="Nome" class="form-control">
            <input type="text" required value="<?php echo $row['SOBRENOME']?>" id="sobrenome" name="sobrenome" placeholder="Sobrenome" class="form-control">
            <input type="email" required value="<?php echo $row['EMAIL']?>" name="email" placeholder="E-mail" class="form-control">
            <?php 
                if($row['PERMISSAO'] == 0){
                    ?>
            ...
            <?php 
                if($row['ATIVO'] == 0){
                    ?>
                    <select name="ativo" required id="permisao" class="form-control">
            ...
            <button type="button" class="btn btn-danger" id="delete" style="margin-top:10px">Deletar usuario</button>
        </form>
    </div>
<div>
</div>
    ...
    $(function(){
        var opc = 0;
            ...
            $("#delete").click(function(){
                var i = confirm("Realmente deseja remove-lo");
                if(i){
                    window.location.href = "../../php/delete professor.php?codigo=<?php echo $row['CPF']?>"
                }
            });
            $('.fa').click(function(){
